        ...
        <tbody>
            <?php if($villes): ?>
                <?php foreach($villes as $ville): ?>
                    <tr id="station_id_<?=$ville->id;?>">
                        <td style="text-align:center" hidden><?= $ville->id ?></td>
                        <td style="text-align:center"><?= $ville->name ?></td>
                        <td style="text-align:center">
                            <a href="javascript:void(0)" id="edit-ville" data-id="<?=$ville->id?>" class="btn btn-info"> <i class="fa fa-edit"></i> Modifier</a>
                            <a href="javascript:void(0)" id="delete-ville" data-id="<?=$ville->id?>" class="btn btn-danger delete-user"> <i class="fa fa-trash"></i> Supprimer</a>
                        </td>
                    </tr>
                <?php endforeach;?>
            <?php endif;?>        
       ...
       $('#villeForm').validate({
           submitHandler: function(form){
               ...
               $.ajax({
                   ...
                   success: function(res){
                    var ville='<tr id="ville_id_'+ res.data.id + '"><td>' + res.data.id + '</td><td>' + res.data.name + '</td>'; 
                    ville+= '<td><a href="javascript:void(0)" id="edit-ville" data-id="' + res.data.id + '"class="btn btn-info">Modifier</a><a href="javascript:void(0)" id="delete-ville" data-id="' + res.data.id + '"class="btn btn-danger delete-user">Supprimer</a></td></tr>';
