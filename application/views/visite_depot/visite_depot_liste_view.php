...
<div class="container main_content">
    <h2>Liste des visites des dépôts aviation</h2>
    <div class="card">
        <!--Barre de recherche et pagination-->
        <div id="pagination"></div>
        <div class="form-group row">
            <div class="col-xs-3">
                <input type="text" name="depot_name" id ="station_name" class="field-divided form-control input-xs" placeholder="Rechercher un depôt aviation..." />
            </div>
            <div class="col-xs-3">
...
<script>
    ...
    $(document).ready(function(){
        ...
        function loadPagination(pagno){
            $.ajax({
                url:SITEURL+'/visite_depot/loadRecord/'+pagno,
                type: 'get',
                dataType: 'json',
        ...
        function createTable(result,sno){
            ...
            for(index in result){
                ...
                var gerant = result[index].gerant;
                var visiteur = result[index].visiteur;
                var depot = result[index].nom;
                ...
                tr += "<td>"+ gerant +"</td>";
                tr += "<td>"+ visiteur +"</td>";
                tr+= "<td> <a href='<?php echo base_url()?>visite_depot/reponse_visite?id_visite="+id+"' id='view' class='fa fa-eye btn' data-id='"+id+"'> </a> <a class='fa fa-file-pdf-o' id='download' data-id='"+id+"'></a></td>";
                tr += "</tr>";
        ...
        $('body').on('click','#rechercher',function(){
            ...
            if(station!='' || visiteur!='' || date!=''){
                $.ajax({
                    type:"Post",
                    url:SITEURL + "visite_depot/search_visite",
                    data:{
                        depot_name : depot,
