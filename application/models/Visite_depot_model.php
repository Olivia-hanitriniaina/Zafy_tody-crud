...
class Visite_depot_model extends CI_model{
    ...
    public function get_count(){
        try{
           $this->db->select('*');
           $this->db->from('VisiteDepotAviation');
    ...
    public function get_all_visites($limit,$start){
        try{
            $this->db->select('VisiteDepotAviation.visite_id,Visite.date,Visite.time,Gerant.nom_complet AS gerant,Visiteur.nom_complet AS visiteur,Localisation.nom');
            $this->db->from('VisiteDepotAviation');
            $this->db->join('Visite','Visite.id=visite_id','inner');
            $this->db->join('Localisation','Localisation.id=localisation_id','inner');
            $this->db->join('Utilisateur AS Visiteur','Visiteur.id=visiteur_id','inner');
    ...
    public function get_all_visites_by_id($id){
        try{
            ...
            $this->db->join('Utilisateur AS Visiteur','Visiteur.id=visiteur_id','inner');
            $this->db->join('Utilisateur AS Gerant','Gerant.id=responsable_id');
            $this->db->where('VisiteDepotAviation.visite_id',$id);
            $query=$this->db->get();
            return $query->result();
        }catch(Exception $e){
            show_error($e->getMessage().'------'.$e->getTraceAsString());
